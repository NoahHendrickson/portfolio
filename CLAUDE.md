...
Project pages come in two layouts. `ProjectLanding.tsx` is the generic one — stats row, `label`-railed sections, stack and status grids — and is still on the cream theme. `ProjectStory.tsx` is the screenshot-led one, drawn from the **story frames** of the July 2026 file (`5mDT1eQf2KBcET9dh6kPXd`, section `58:3085` — D2 Stat Builder, Phanttom, The Forge): a centered 688px title block, then centered screenshot rows — a `columns` section puts shots side by side with Body/L captions underneath, a `row` section seats one shot beside its 18px caption — plus an optional wall of chat screenshots on a tinted band and a closing orange card. A project opts into it by setting `landing` in `projects.ts`.

The page has no padding of its own — each section pads itself on a **120px gutter with 80px rows**, and the page container's 24px gap sits between them, because the feedback band's tint runs edge to edge. Row and column widths are px at the 1272px content width, rendered as `fr` shares of a proportional row, so shots, captions and gaps scale together. `landing.hero.shot` is the app window under the copy block (Phanttom); the other two run the title block alone. The Back pill at the top is kept deliberately even though the frames drop it. The one overlapping composition — the table frame with the sort panel hanging off its bottom-right — is a `COMPOSITION` box of percentages that overhangs its 409px column into the trailing gutter, and the wrapper carries the group's aspect ratio so the caption clears the panel rather than stacking against the frame. The Forge's outro sets `large` for the file's 40px heading (`display-l`); the other cards stay on Heading/L.

Figma sizes the browser frames by fixed height and clips the capture inside; our `browser-bar.png` renders a shade shorter than Figma's bar, so the shots' `aspect` values give that difference back to the image window and the frames land on their designed heights. `LandingShot.zoom` is for the shots the file crops into rather than fits (the table runs at 1.102× its frame). `FRAME_BORDER` / `FRAME_BG` / `OVERLAY_BORDER` are the screenshots' own chrome, not theme surfaces, so they stay hexes.
